...
_getGlobal('windowModules').forEach(function (name) {
  if (_.isString()) require(name)
})

_getGlobal('windowScripts').forEach(function (val) {
  if (_.isString(val)) {
    require(val)
  } else if (_.isPlainObject(val) && _.has(val, 'name') && _.has(val, 'path')) {
