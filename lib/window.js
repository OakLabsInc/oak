...
class DisplayService extends EventEmitter2 {
  constructor (core, opts, callback = function () {}) {
    ...
    this.id = opts.id || this.instance.id
    ...
    ipcMain
      ...
      .on('_reload', (ev, d) => filterWindow(ev, d, _this.reload))
      .on('_debug', (ev, d) => filterWindow(ev, d, _this.debug))
      .on('_hide', (ev, d) => filterWindow(ev, d, _this.hide))
      .on('_show', (ev, d) => filterWindow(ev, d, _this.show))
      .on('_focus', (ev, d) => filterWindow(ev, d, _this.focus))
      .on('_proceed', (ev, d) => filterWindow(ev, d, _this.proceed))
      .on('_ready', (ev, d) => filterWindow(ev, d, _this.ready))

    function filterWindow (ev, data, fn) {
